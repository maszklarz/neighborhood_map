...
There are two approaches for the insallation: production and development. The production build is optimized and includes the service worker functionality. Use this to verify the offline-first approach.
The development build has the service worker turned off. It helps to avoid issues caused by cached data. Use it for development purposes.

# Production Build
1. Clone the git repository
2. Go to the project's main directory and type `npm install`
3. Execute `npm run build` to prepare the production build.
...
## Port Number
In order to change the port number make sure the `src/DBHelper.js` and `package.json` are modified accordingly.


## Usage
The application shows a map and a list of predefined places. Hovering over the place on the list highlights the respective pin on the map. Clicking the pin or the listed place initiates an attempt to complete the data based on external source. So far it uses a Foursquare API. If the data is unavailable or cannot be download for any reason the respective message is displayed in the place description area.
Note, the token used to gather Forsquare data is limited to 50 download attempts a day. If the limit is exceeded, the 429 code value is displayed in the cryptic form of lottery numbers ;-)
...
- src/NytAPI.js
- src/UnsplashAPI.js
The latter two APIs have been tested but they are not used at the moment. Respective boilerplate code has been included in comments.

Additionally there is a module with functions to gather the predefined list of places stored in public/places.json:
- src/DBHelper.js
